...
\begin{defn}[Diﬀusion Carr\'e du Champ]
...
\begin{prop}
    ...
    \end{equation*}
    for every $t \geq 0$ and every $f \in L^1\bc{\mu}$ with finite entropy.
\end{prop}
\begin{proof}
...
\begin{rmk}
    For $L$, we can similarly consider the $L^*$ acting on probability measure on $M$. If $d\nu = pdV$, then by definition of $L^*\nu$,
    \begin{equation*}
        d(L^*\nu) = (L^*p)dV,
    \end{equation*}
    where on the right hand side $L^*p$ coincide with the adjoint of $L$ on $L^2(dV)$. Assume $p \in C^\infty$. Then for any $g \in C^\infty$,
    \begin{align*}
        \int_M gd(L^*\nu) &= \int_M g (L^*p) dV = \int_M (Lg)pdV \\
        &= \int_M p\Delta g dV - \int_M \inn{\nabla W, p\nabla g}dV\\
        &= \int_M g\Delta p dV + \int_M pg\Delta W + \int_M \inn{\nabla W, \nabla p}gdV,\\
        &= \int_M (\Delta p + \op{div} (p \nabla W))gdV.
    \end{align*}
    because $\op{div}(f\nabla W) = \inn{\nabla W, \nabla f} + f\Delta W$. Therefore,
    \begin{equation*}
        L^*p = \Delta p + \op{div} (p \nabla W).
    \end{equation*}
\end{rmk}
