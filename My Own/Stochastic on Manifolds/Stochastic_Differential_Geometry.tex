...
\paragraph{Orthogonal Frame.} When considering $(M,g)$ be a Riemannian manifold with the Levi-Civita connection $\nabla$. Instead of considering $F(M)$, we consider the orthogonal frame $O(M)$, that is, $O(M)_x$ consists of all $u \colon \R^d \sto T_xM$ orthogonal map. The problem is if we can consider the horizontal lift on $O(M)$. To do that, let
\begin{equation*}
    u(t)a = V^i_j(t)a^j \frac{\partial}{\partial x^i}
\end{equation*}
...
\begin{equation*}
    \inn{u(t)e_i,u(t)e_j} = \inn{V^\alpha_i\frac{\partial}{\partial x^\alpha},V^\beta_j\frac{\partial}{\partial x^\beta}} = V^\alpha_ig_{\alpha\beta}V^\beta_j = (U(t)^\top G(t)U(t))_{ij} = \delta_{ij},
\end{equation*}
i.e., $U(t)^\top G(t)U(t) \equiv I$, where $U(t) = (V^i_j(t))$ and $G(t) = (g_{ij}(c(t)))$, the local expression of metric. Note that, by the consistency of metric
\begin{align*}
    \dot{g}_{ij}(c(t)) &= \dot{c}(t) \inn{\frac{\partial}{\partial x^i},\frac{\partial}{\partial x^i}} \\
    ...
    &=\dot{c}^\alpha \Gamma^k_{\alpha i}g_{kj} + \dot{c}^\alpha \Gamma^k_{\alpha j}g_{ik} \\
    &= A^\top G + GA,
\end{align*}
where $A = (A^i_k)$ for $A^i_k(t) = \Gamma^i_{jk}(c(t))\dot{c}^j(t)$. Moreover, because
\begin{equation*}
    \dot{U}(t) + A(t)U(t) = 0,
\end{equation*}
we have
\begin{align*}
    \frac{d}{dt}U^\top GU &= \dot{U}^{\top} G U+U^{\top} \dot{G} U+U^{\top} G \dot{U}\\
    &= -U^\top A^\top G U + U^\top(A^\top G + GA) U - U^{\top} G(AU) = 0.
\end{align*}
Therefore,
\begin{equation*}
    U\top GU \equiv U^{\top}(0) G(0)U(0) = I,
\end{equation*}
...
For any $x \in M$, let $P(x) \colon \R^N \sto T_xM$ be the orthogonal projection by viewing $T_xM \subset \R^N$. Let $P_\alpha(x) = P(x)e_\alpha \in T_xM$ and so $P_\alpha \in \Gamma(TM)$. 
\begin{rmk}
    For the smoothness of $P_\alpha$, first we can equip $M$ with the induced metric. Then on each local chart, there exists a smooth orthonormal frames $E_i$ on $TM$, which implies that
    \begin{equation*}
        P_\alpha = \sum_i \inn{e_\alpha,E_i}E_i.
    \end{equation*}
    So $P_\alpha$ is smooth.
\end{rmk}

Define $P^H_\alpha \in \Gamma(TF(M))$ as
\begin{equation*}
...
\begin{rmk}
    ...
    Then by above,
    \begin{equation*}
        X_t = X_0 + \int_0^s U_se_i \circ dW^i_s.
    \end{equation*}
    In formally, we just denote
    \begin{equation*}
        X_t = X_0 + \int_0^s U_s \circ dW_s.
    \end{equation*}
    Similarly, because $dX_t = P_\alpha(X_t) \circ dX^\alpha_t$, we can informally write
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
    Let $U_t$ be its unique solution, so $U_t$ is clearly a horizontal lift since $P^H_\alpha \in \Gamma{F(M)}$. It suffices to show $\pi(U_t)= X_t$. Let
    \begin{equation*}
...
\section{Martingales on Submanifolds}

Let $M,N$ be two manifolds equipped with connection $\nabla^M,\nabla^N$ respectively, and $M \subset N$ be a submanifold. Consider the second fundamental form $\mathrm{II} \colon \Gamma(TM) \times \Gamma(TM) \sto \Gamma(TN|_M)$ defined as
\begin{equation*}
    \mathrm{II}(V,W) = \nabla^N_VW - \nabla^M_VW,
\end{equation*}
which is obviously bilinear over $C^\infty(M)$, so $\mathrm{II}$ is a $TN|_M$-valued $(0,2)$-tensor field on $M$. 
\begin{rmk}
    A direct result of this is, $\mathrm{II}(V,W)(p)$ is fully determined by $V_p$ and $W_p$
\end{rmk}
Moreover, for a connection $\nabla$, let
\begin{equation*}
    T(V,W) = \nabla_VW - \nabla_WV - [V,W].
\end{equation*}
We have
\begin{equation*}
    \mathrm{II}(V,W) - \mathrm{II}(W,V) = T^N(V,W) - T^M(V,W),
\end{equation*}
so $\mathrm{II}$ is symmetric if $\nabla^M,\nabla^N$ are torsion-free. If $\mathrm{II} \equiv 0$, then $M$ is called totally geodesic.

\begin{prop}
    If $M$ is totally geodesic, then any $\nabla^M$-geodesic in $M$ is also a $\nabla^N$-geodesic in $N$. When $\nabla^M,\nabla^N$ are torsion-free, the converse is also true.
\end{prop}

\begin{lem}
    Let $M \subset \R^N$ be a submanifold and $f \colon M \sto \R^N$ be the coordinate function on $M$ with lift $\widetilde{f} = f \circ \pi$ to the frame bundle $F(M)$. Then
    \begin{equation*}
        H_iH_j \widetilde{f}(u) = \mathrm{II}(ue_i,ue_j)
    \end{equation*}
\end{lem}
\begin{proof}
    Let $u_t$ be the horizontal lift starting from $u_0 = u$ of a curve $x_t$ with $x_0 = \pi(u)$ and $\dot{x}_0 = ue_i$, i.e., $\dot{u}(0)=H_i(u)$, and $V(x_t) = u_te_j$ is a $\nabla^M$-parallel along $x_t$. So 
    \begin{equation*}
        \nabla^M_{\dot{x}_t} V = 0~\Rightarrow~ \nabla^M_{ue_i}V = 0.
    \end{equation*}
    It implies that
    \begin{equation*}
        \mathrm{II}(ue_i,ue_j) = \nabla^{\R^N}_{ue_i}V.
    \end{equation*}
    On the other hand, recall when viewing $T_xM \subset \R^N$,
    \begin{equation*}
        H_j \widetilde{f}(u_t) = u_te_j = V(x_t).
    \end{equation*}
    Therefore,
    \begin{equation*}
        H_iH_j\widetilde{f}(u) = \lv{\frac{d}{dt}}_{t = 0} V(x_t) = \nabla^{\R^N}_{ue_i}V = \mathrm{II}(ue_i,ue_j)
    \end{equation*}
    for viewing $V(x_t) = (V^1(x_t),\cdots,V^N(x_t)) \in \R^N$. \qedhere
\end{proof}

\begin{thm}
    Using notations as above and assume $\dim M = l$ and $\dim N = d$. Suppose that $X$ is a semimartingale on $M \subset N$. Let $U^M$ be a horizontal lift of $X$ in $F(M)$ with the corresponding $\R^d$-valued anti-development $W^M$, and $U^N$ be a horizontal lift of $X$ in $F(N)$ with the corresponding $\R^l$-valued anti-development $W^N$. Then
    \begin{equation*}
        W^N_t = \int_0^t (U^N_s)^{-1}U^M_sdW^M_s + \frac{1}{2} \int_0^t(U^N_s)^{-1}\mathrm{II}(dX_s,dX_s).
    \end{equation*}
\end{thm}
\begin{proof}
    First,
    \begin{equation*}
        \mathrm{II}(dX_s,dX_s) = \mathrm{II}(U^M_se_i,U^M_se_j)d\inn{(W^M)^i,(W^M)^j}_s.
    \end{equation*}
    So we need to show
    \begin{equation*}
        W^N_t = \int_0^t (U^N_s)^{-1}U^M_se_id(W^M)_s + \int_0^t(U^N_s)^{-1}\mathrm{II}(U^M_se_i,U^M_se_j)d\inn{(W^M)^i,(W^M)^j}_s.
    \end{equation*}

    First, assume $N = \R^l$. WTLG, we can choose $U^N_0 = I$. In such case, we have
    \begin{equation*}
        W^N = X - X_0,\quad U^N \equiv I.
    \end{equation*}
    For simplicity, denote $U = U^M$ and $W = W^M$. Let $f \colon M \sto \R^l$ be the coordinate map and $\widetilde{f} = f \circ \pi$. Then we have been shown
    \begin{equation*}
        X_t = X_0 + \int_0^t H_i\widetilde{f}(U_s)dW^i_s + \frac{1}{2}\int_0^tH_iH_j\widetilde{f}(U_s) d\inn{W^i,W^j}_s.
    \end{equation*}
    Note that
    \begin{equation*}
        H_i \widetilde{f}(u)=u e_i, \quad H_i H_j \widetilde{f}(u)=\mathrm{II}(ue_i,ue_j).
    \end{equation*}
    Therefore,
    \begin{equation*}
        W^N_t = X_t - X_0 = \int_0^t U_s d W_s+\frac{1}{2} \int_0^t \mathrm{II}\left(d X_s, d X_s\right).
    \end{equation*}

    Next, for general $N$, assume $N \subset \R^N$ embedded manifold. Let $\mathrm{II}^{M}$ and $\mathrm{II}^N$ be the second fundamental forms of $M$ and $N$ in $\R^N$ respectively. Then
    \begin{align*}
        X_t&=X_0+\int_0^t U^M_s d W^M_s+\frac{1}{2} \int_0^t \mathrm{II}^{M}\left(d X_s, d X_s\right) \\
        X_t&=X_0+\int_0^t U^N_s d W^N_s+\frac{1}{2} \int_0^t \mathrm{II}^{N}\left(d X_s, d X_s\right),
    \end{align*}
    where the two equations are viewed in $\R^N$, $T_xM = \R^l$ and $T_xN = \R^d$. Note that
    \begin{equation*}
        \mathrm{II}(X,Y) = \mathrm{II}^M(X,Y) - \mathrm{II}^N(X,Y).
    \end{equation*}
    By above, $dX_t = U^N_s dW^N_s + \frac{1}{2} \mathrm{II}^{N}\left(d X_s, d X_s\right)$,
    \begin{align*}
        W^N_t &= \int_0^t (U^N_s)^{-1}dX_t - \frac{1}{2}\int_0^t (U_s^N)^{-1}\mathrm{II}^{N}\left(d X_s, d X_s\right) \\
        &= \int_0^t (U^N_s)^{-1}U^M_s d W^M_s + \frac{1}{2}\int_0^t (U^N_s)^{-1}\mathrm{II}^{M}\left(d X_s, d X_s\right) \\
        & \quad - \frac{1}{2}\int_0^t (U_s^N)^{-1}\mathrm{II}^{N}\left(d X_s, d X_s\right) \\
        &= \int_0^t (U^N_s)^{-1}U^M_s d W^M_s + \frac{1}{2} \int_0^t(U^N_s)^{-1}\mathrm{II}(dX_s,dX_s). \qedhere
    \end{align*}
\end{proof}

\begin{cor}
    Suppose that $M \subset N$ is a totally geodesic submanifold of $N$. Then every $\nabla^M$-martingale on $M$ is also a $\nabla^N$-martingale on $N$.
\end{cor}

For $x \in M$, define
\begin{equation*}
    \ran \mathrm{II}_x \defeq \bb{\mathrm{II}(X,Y)(x) \colon X,Y \in \Gamma(TM)} \subset T_xN.
\end{equation*}
$\mathrm{II}$ is called independent of $TM$ if $\ran \mathrm{II}_x \cap T_xM = \bb{0}$ for every $x \in M$. Note that for Riemannian submanifold, $\mathrm{II}$ is obviously independent of $TM$.

\begin{cor}
    Suppose that $M \subset N$ such that $\mathrm{II}$ is independent of $TM$. Then a $\nabla^N$-martingale on $N$ which lives on $M$ is also a $\nabla^M$-martingale on $M$.
\end{cor}
\begin{proof}
    First, $W^M$ is clearly a continuous semi-martingale, so it can be uniquely decomposed into
    \begin{equation*}
        W_t = M_t + A_t,
    \end{equation*}
    where $M$ is a local martingale and $A$ is a process of bounded variation with $A_0 = 0$. It suffices to prove $A = 0$. By above,
    \begin{equation*}
         W^N_t = \int_0^t (U^N_s)^{-1}U^M_s d W^M_s +  \int_0^t(U^N_s)^{-1}\bc{ U_s^MdA_s + \frac{1}{2}\mathrm{II}(dX_s,dX_s)}.
    \end{equation*}
    Because $W^N$ is a local martingale,
    \begin{equation*}
        \int_0^t(U^N_s)^{-1}\bc{ U_s^MdA_s + \frac{1}{2}\mathrm{II}(dX_s,dX_s)} = 0
    \end{equation*}
    and so
    \begin{equation*}
        U_s^MdA_s + \frac{1}{2}\mathrm{II}(dX_s,dX_s) = 0.
    \end{equation*}
    However,
    \begin{equation*}
        U_s^MdA_s \in T_{\pi(U_s^M)}M,\quad \mathrm{II}(dX_s,dX_s) \in \ran \mathrm{II}_{\pi(U_s^M)}.
    \end{equation*}
    The independence implies that $dA_s = 0$, and so $A = 0$. \qedhere
\end{proof}

\begin{exam}
    \begin{enumerate}[label=(\arabic{*})]
        \item Let $M \subset N$ be a Riemannian hypersurface. It is called (strictly) convex if the scalar fundamental form is (strictly) positive definite at each point. Assume $M$ is strictly convex. Because $\mathrm{II}$ is independent of $TM$, any $\nabla^N$-martingale on $N$ which lies on $M$ is also a $\nabla^M$-martingale. It implies that
        \begin{equation*}
            \mathrm{II}(dX_s,dX_s) = 0.
        \end{equation*}
        However, because of the strict positivity, $dX_s = 0$, and so $X$ is constant.

        \item Let $M$ be a Riemannian manifold of dimension $\R^d$. A semimartingale $X$ on $M$ is called a Brownian motion if its anti-development $W$ is a $d$-dimensional standard Brownian motion. Let $M \subset \R^N$ be a minimal submanifold, i.e., the mean curvature field $H = \tr_g \mathrm{II} = 0$. Then a Brownian motion on $M$ is a local martingale on $\R^N$, because
        \begin{equation*}
            \mathrm{II}\left(U_t e_i, U_t e_j\right) d\left\langle W^i, W^j\right\rangle_t=\sum_i\mathrm{II}\left(U_t e_i, U_t e_i\right) d t=H\left(X_t\right) d t=0.
        \end{equation*}
        Note that for Riemannian manifold, we consider $O(M)$, and so $\bb{U_te_i}$ is orthonormal in $T_{\pi(U_t)}M$.
    \end{enumerate}
\end{exam}

\begin{prop}
    Suppose that $M \subset N$ be totally geodesic. For any $x \in M$, there exists a neighborhood $O$ of $p$ in $N$ such that if $X$ is an $\nabla^N$-martingale in $O$ on $[0,T]$ with $X_T \in M$, then $X_t \in M$ for all $t \in [0,T]$.
\end{prop}

\begin{thm}
    Let $M$ be a manifold with a connection. For any $x \in M$, there exists a neighborhood $O$ of $x$ such that: if $(X_t)_{t \in [0,T]}$ and $(Y_t)_{t \in [0,T]}$ are two martingales on $V$ such that $X_T = Y_T$, then $X_t = Y_t$ for all $t \in [0,T]$.
\end{thm}
